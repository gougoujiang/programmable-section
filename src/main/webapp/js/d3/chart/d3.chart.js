...
(function () {
			...
			this.init = function (options) {
				...
				containerWidth = jQuery(_container).width();
				width = containerWidth - margin.left - margin.right;
				...
				_svgContainer// mouse event not working on _chartCanvas
					...
					.on('mousemove', function () {
						var mouse = d3.mouse(this);
						var mX = mouse[0] - margin.left, mY = mouse[1] - margin.top;
                                                mX = Math.round(mX); //In case the view is zoomed, the mX might not be integer.
                                                mY = Math.round(mY);
			...
			function adjustChartHeight() {
				...
				jQuery('.loader').height(containerHeight);
			...
			function getLookupMap(graph, xScale) {
				...
				var map = [];
                                // BugFix(temp): The index calc seems useless.
                                //var startIndex = _.sortedIndex(graph.data, xScale.domain()[0], function (v) { return v.DateTime ? moment(v.DateTime).valueOf() : moment(v).valueOf(); });
				//var endIndex = _.sortedIndex(graph.data, xScale.domain()[1], function (v) { return v.DateTime ? moment(v.DateTime).valueOf() : moment(v).valueOf(); });
